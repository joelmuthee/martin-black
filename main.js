...
(async function () {
  ...
  const ANALYTICS_KEY = 'martinblack_analytics';
  ...
  function buildSizePills() {
    ...
    // Martin Black sizes are all EU shoe sizes (numeric) — one sorted list, no optgroups.
  ...
  function showSuspended() {
    ...
    const shopName = settings.shopName || settings.businessName || 'Martin Black';
    const tagline = settings.tagline || 'Online Shoe Shop';
    const igHandle = (settings.instagramHandle || 'martinblackshoesandtrends').replace(/^@/, '');
    const igLink = igHandle ? ('https://www.instagram.com/' + igHandle + '/') : '';
    const logoUrl = 'images/logo-nav.png';
